 ...
 * Rather than implementing the loading and rendering of media directly, {@link ExoPlayer2} instead
 * delegates this work to one or more {@link TrackRenderer}s, which are injected when the player
 * is prepared. Hence {@link ExoPlayer2} is capable of loading and playing any media for which a
 ...
 *
 * <p>The figure below shows the {@link ExoPlayer2} threading model.</p>
 ...
 * </li>
 * <li>Registered {@link Listener}s are invoked on the thread that created the {@link ExoPlayer2}
 * instance.</li>
 ...
 * <h3>Player state</h3>
 *
 * <p>The components of an {@link ExoPlayer2}'s state can be divided into two distinct groups. State
...
public interface ExoPlayer2 {
  ...
  public static final class Factory {
    ...
    /**
     * Obtains an {@link ExoPlayer2} instance.
     * <p>
     ...
     *     not due to a user action such as starting playback or seeking).
     */
    public static ExoPlayer2 newInstance(int rendererCount, int minBufferMs, int minRebufferMs) {
     ...
     *     {@link #prepare(TrackRenderer[])}.
     */
    public static ExoPlayer2 newInstance(int rendererCount) {
  ...
  public interface Listener {
    /**
     * Invoked when the value returned from either {@link ExoPlayer2#getPlayWhenReady()} or
     * {@link ExoPlayer2#getPlaybackState()} changes.
     *
     * @param playWhenReady Whether playback will proceed when ready.
     * @param playbackState One of the {@code STATE} constants defined in the {@link ExoPlayer2}
     *     interface.
     */
    void onPlayerStateChanged(boolean playWhenReady, int playbackState);
    /**
     * Invoked when the current value of {@link ExoPlayer2#getPlayWhenReady()} has been reflected
     * by the internal playback thread.
     * <p>
     * An invocation of this method will shortly follow any call to
     * {@link ExoPlayer2#setPlayWhenReady(boolean)} that changes the state. If multiple calls are
     ...
     * {@link ExoPlayer2#STATE_IDLE} immediately after this method is invoked. The player instance
     * can still be used, and {@link ExoPlayer2#release()} must still be called on the player should
  ...
  /**
   * A component of an {@link ExoPlayer2} that can receive messages on the playback thread.
   * <p>
   * Messages can be delivered to a component via {@link ExoPlayer2#sendMessage} and
   * {@link ExoPlayer2#blockingSendMessage}.
   */
   ...
   *     value that was passed to the {@link ExoPlayer2.Factory#newInstance} method.
   */
  ...
  /**
   * Whether the current value of {@link ExoPlayer2#getPlayWhenReady()} has been reflected by the
   ...
   * {@link ExoPlayer2#STATE_IDLE}. The player instance can still be used, and
   * {@link ExoPlayer2#release()} must still be called on the player if it's no longer required.
   * <p>
   ...
   *
   * @return The duration of the track in milliseconds, or {@link ExoPlayer2#UNKNOWN_TIME} if the
   ...
   *     or {@link ExoPlayer2#UNKNOWN_TIME} if no estimate is available.
   */
